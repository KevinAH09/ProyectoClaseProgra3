...
public class ArchivoRelacionadoController {
    ...
    public ResponseEntity<?> findById(@PathVariable(value = "id") Long id) {
        try {
            ...
            if (archivoRelacionadoResult.isPresent()) {
                ArchivoRelacionadoDTO resultArchivoRelacionadoDto = MapperUtils.DtoFromEntity(archivoRelacionadoResult.get(), ArchivoRelacionadoDTO.class);
                return new ResponseEntity<>(resultArchivoRelacionadoDto, HttpStatus.OK);
            } else {
    ...
    public ResponseEntity<?> findByTramiteRegistrado(@PathVariable(value = "id")Long  id) {
        try{
            ...
            if(result.isPresent()){
                List<ArchivoRelacionadoDTO> resultArchivoRelacionadoDto = MapperUtils.DtoListFromEntityList(result.get(), ArchivoRelacionadoDTO.class);
                return new ResponseEntity<>(resultArchivoRelacionadoDto, HttpStatus.OK);
            }
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }catch(Exception ex){
    ...
    public ResponseEntity<?> update(@PathVariable(value = "id") Long id, @RequestBody ArchivoRelacionado archivoRelacionadoModified) {
        try {
            ...
            if (archivoRelacionadoUpdated.isPresent()) {
                ArchivoRelacionadoDTO archivoRelacionadoDto = MapperUtils.DtoFromEntity(archivoRelacionadoUpdated.get(), ArchivoRelacionadoDTO.class);
                return new ResponseEntity<>(archivoRelacionadoDto, HttpStatus.OK);
            } else {
